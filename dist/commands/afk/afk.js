export default {
    ...
    run: async (client, message, args) => {
        ...
        const data = (await client.database.prepare('SELECT * FROM afk WHERE user_id = ?').get(userId));
        if (data) {
            return message.reply({
                content: `**${message.author.tag}**, you are already AFK. Your current AFK reason is: "${data.reason}" set at <t:${Math.floor(data.time / 1000)}:R>`
            });
        }
        else {
